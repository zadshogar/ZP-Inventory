...
export default function App() {
  ...
  return (
    <div style={{ background:C.bg, minHeight:'100vh', color:C.text, fontFamily:'system-ui,sans-serif', fontSize:15, paddingBottom:60 }}>
      ...
      {tab==='alerts' && (
        <div style={{padding:14}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:14}}>
            {[{val:parts.length,lbl:'TOTAL PARTS',c:C.text},{val:totalUnits,lbl:'TOTAL UNITS',c:C.text},
              {val:lowParts.length,lbl:'LOW STOCK',c:C.accent},{val:criticalParts.length,lbl:'CRITICAL',c:C.red}
            ].map(({val,lbl,c}) => (
              <div key={lbl} style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:12,textAlign:'center'}}>
            ...
            : alertParts.map(p => (
              <div key={p.id} style={{display:'flex',alignItems:'center',gap:9,padding:'9px 12px',marginBottom:7,borderRadius:8,
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{fontSize:11,color:C.muted,marginTop:1}}>{p.qty} {p.unit||'pcs'} · {p._st==='critical'?'CRITICAL':'LOW STOCK'}</div>
                </div>
                <button onClick={()=>setTxModal({part:p,type:'add'})}
                  style={{padding:'6px 10px',background:'rgba(61,224,138,.13)',color:C.green,
                    border:`1px solid rgba(61,224,138,.28)`,borderRadius:8,fontWeight:700,fontSize:11,cursor:'pointer',fontFamily:'system-ui,sans-serif'}}>
                  Restock
                </button>
              </div>
            ))
      ...
      {deleteTarget && (
        <ConfirmDialog title={`Delete "${deleteTarget.name}"?`}
